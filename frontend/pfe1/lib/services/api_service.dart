...
static Future<List<Moto>> getClientMotos() async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print("Erreur lors de la récupération des motos: ${response.statusCode}");
      return [];
    }
  } catch (e) {
...
static Future<Map<String, dynamic>> addMoto(int modelId, String? image) async {
  try {
    if (!isLoggedIn() || currentUser == null) {
...
static Future<Map<String, dynamic>> deleteMoto(int motoId) async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      return {
        "success": false,
        "message": data['message'] ?? "Erreur lors de la suppression de la moto"
      };
    }
  } catch (e) {
    return {"success": false, "message": "Exception: $e"};
...
static Future<Map<String, dynamic>> getCart() async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      return {
        "success": false,
        "message": "Erreur lors de la récupération du panier: ${response.statusCode}"
      };
    }
  } catch (e) {
...
static Future<Map<String, dynamic>> addToCart(int schemaId, int quantity) async {
  try {
    print("ApiService - addToCart called with schemaId: $schemaId, quantity: $quantity");
    ...
    print("ApiService - Request body: ${jsonEncode({
      'schema_id': schemaId,
      'quantity': quantity,
    })}");
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      final Map<String, dynamic> data = 
        response.body.isNotEmpty ? jsonDecode(response.body) : {"message": "Erreur inconnue"};
      return {
        "success": false,
...
static Future<Map<String, dynamic>> updateCartItemQuantity(int productId, int quantity) async {
  try {
    if (!isLoggedIn() || currentUser == null) {
      return {"success": false, "message": "Utilisateur non connecté"};
    }
...
static Future<List<Schema>> getCompatibleSchemas(int motoId) async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print("Erreur lors de la récupération des schémas: ${response.statusCode}");
...
static Future<Map<String, dynamic>> createCommandeFromCart() async {
  try {
    ...
    print("ApiService - Sending POST request to $baseUrl/commandes/create-from-cart");
    ...
    print("ApiService - Response status: ${response.statusCode}");
    print("ApiService - Response body: ${response.body.substring(0, min(100, response.body.length))}...");
    ...
    if (response.body.startsWith('{') || response.body.startsWith('[')) {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        return {
          "success": false,
          "message": data['message'] ?? "Erreur lors de la création des commandes"
        };
      }
    } else {
      ...
      return {
        "success": false,
        "message": "Le serveur a renvoyé une réponse non-JSON. Vérifiez les logs pour plus de détails."
      };
    }
  } catch (e) {
