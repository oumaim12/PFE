<x-app-layout>
    ...
    <div class="content-panel">
        <div class="panel-header flex justify-between items-center">
            <h3 class="panel-title">{{ $schema->nom }} <span class="text-polished-chrome/70 text-sm font-normal ml-2">(Version: {{ $schema->version }})</span></h3>
            <div class="flex space-x-2">
                ...
                @if($schema->enfants->count() == 0 && $schema->commandes->count() == 0)
                    <form action="{{ route('schemas.destroy', $schema->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette pièce ?');">
                        @csrf
                        @method('DELETE')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <div class="mb-3">
                                    ...
                                    <span class="text-white font-medium">{{ $schema->created_at->format('d/m/Y H:i') }}</span>
                                </div>
                                <div class="mb-3">
                                    <span class="block text-polished-chrome/70 text-sm">Dernière mise à jour</span>
                                    <span class="text-white font-medium">{{ $schema->updated_at->format('d/m/Y H:i') }}</span>
                                </div>
                                <div class="mb-3">
                    ...
                    @if($commandesRecentes && $commandesRecentes->count() > 0)
                        <div class="content-panel">
                            <div class="panel-header flex justify-between items-center">
                                ...
                                @if($totalCommandes > 5)
                                    <span class="text-polished-chrome/70 text-sm">Affichant 5 sur {{ $totalCommandes }}</span>
                                @endif
                            </div>
                            <div class="panel-body p-0">
                                <table class="moto-table">
                ...
                <div>
                        ...
                        <div class="bg-deep-metal rounded-lg p-4 mb-6">
                            ...
                            <div class="space-y-3">
                                <div class="p-3 rounded bg-carbon-fiber">
                                    ...
                                    <a href="{{ route('schemas.show', $schema->parent->id) }}" class="flex items-center text-exhaust-blue hover:text-white transition-colors">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="bg-deep-metal rounded-lg p-4">
                        ...
                        <div class="space-y-3">
                            <a href="{{ route('schemas.edit', $schema->id) }}" class="block w-full p-3 bg-exhaust-blue hover:bg-blue-700 text-white rounded-md text-center transition-colors">
                                Modifier cette pièce
                            </a>
                            <a href="{{ route('commandes.create') }}?schema_id={{ $schema->id }}" class="block w-full p-3 bg-fuel-yellow hover:bg-yellow-600 text-white rounded-md text-center transition-colors">
                                Créer une commande
                            </a>
                            <a href="{{ route('schemas.create') }}?parent_id={{ $schema->id }}" class="block w-full p-3 bg-engine-red hover:bg-red-700 text-white rounded-md text-center transition-colors">
